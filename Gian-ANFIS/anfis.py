...
class AnfisNetClassifier(torch.nn.Module):
    '''
        Added by Gian.
        I want to output class scores (logits) from the consequent layer. 
        The number of classes is determined when instancing this class.
        We only use PlainLayer: we learn consequent parameters only through backpropagation
        as not to have problems with gels or lstq when performing LSE.
        The 'coeff' matrix containing the parameters of the consequent layer has now shape
        number of rules * number of input variables +1 * number of classes.
        Number of class is both num_classes and n_out in the code.
        Note how the total number of rules is computed by multiplying the number of membership functions 
        for each variable together.
        A softmax layer is defined in order to compute logits.
        For training, look at the experimental.py file.
        
    '''
    def __init__(self, invardefs, num_classes):
        super(AnfisNetClassifier, self).__init__()
        self.num_classes = num_classes
        varnames = [v for v, _ in invardefs]
        mfdefs = [FuzzifyVariable(mfs) for _, mfs in invardefs]
        self.num_in = len(invardefs)
        self.num_rules = np.prod([len(mfs) for _, mfs in invardefs])
        cl = PlainConsequentLayer(self.num_in, self.num_rules, self.num_classes)
        self.layer = torch.nn.ModuleDict(OrderedDict([
            ('fuzzify', FuzzifyLayer(mfdefs, varnames)),
            ('rules', AntecedentLayer(mfdefs)),
            # normalisation layer is just implemented as a function.
            ('consequent', cl),
            # softmax layer is just implemented as a function in the 'forward' method of this class.
            ]))
    ...
    def forward(self, x):
        '''
            ...
            as this might be useful for comprehension/debugging.
        '''
        # Apply the fuzzification layer:
        # output shape [n_cases, n_in, n_mfs]
        self.fuzzified = self.layer['fuzzify'](x)
        # Apply the antecedent layer to calculate firing strengths:
        # output shape [n_cases, n_rules]
        self.raw_weights = self.layer['rules'](self.fuzzified)
        ...
        # Apply the consequent layer to get rule outputs:
        # output [n_cases, n_out, n_rules]
        self.rule_tsk = self.layer['consequent'](x)

        # Compute the sum over rules for each output variable:
        # shape [n_cases, n_out, 1]
        tsk_sum = torch.bmm(self.rule_tsk, self.weights.unsqueeze(2))
    
        logits = tsk_sum.squeeze(-1)
