...
class SigmoidMembFunc(torch.nn.Module):
    def __init__(self, a, c):
        ...
        self.a = torch.nn.Parameter(torch.tensor([a], dtype=torch.float))
        self.c = torch.nn.Parameter(torch.tensor([c], dtype=torch.float))

    def forward(self, x):
        return 1 / (1 + torch.exp(-self.a * (x - self.c)))
