...
# Function that does a grid search across different parameters for the indicators and finds the ones with
# highest hurst coefficient. 
# Give two Hurst values within 0.01 of each other, the smaller parameter is kept in order to have the indicators 
...
def grid_search_hurst(df):
    ...
    for ema_short_period in range(10, 25, 1):
        for ema_long_period in range(26, 60, 2):
            ...
            ema_difference_column = f'EMA{ema_short_period}_EMA{ema_long_period}_Difference_Normalized'
            df_emadn = df_emadn.dropna(subset=[ema_difference_column])
